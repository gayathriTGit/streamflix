...
export function Component() {
  ...
  const handlePlayerReady = function (player: Player): void {
    ...
    player.on("timeupdate", () => {
      setPlayerState((draft) => {
        return { ...draft, playedSeconds: Number(player?.currentTime?.()) };
      });
    });

    player.one("durationchange", () => {
      setPlayerInitialized(true);
      setPlayerState((draft) => ({ ...draft, duration: Number(player?.duration?.()) }));
    });
  ...
  const handleVolumeChange: NonNullable<SliderProps["onChange"]> = (_, value) => {
